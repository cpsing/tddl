        ...
        try {
            ...
            if (connRestrictSlot != null) {
                if (!connRestrictSlot.allocateConnection(timeoutInMillis)) {
                    // 阻塞超时
                    throw new NestedSQLException("No connection available for '" + connKey
                                                 + "' within configured blocking timeout (" + timeoutInMillis + "[ms])");
                }
            }
        } catch (InterruptedException e) {
            throw new NestedSQLException("Allocate connection for '" + connKey
                                         + "' interrupted within configured blocking timeout (" + timeoutInMillis
                                         + "[ms])", e);
        } catch (RuntimeException e) {
            throw new NestedSQLException("Allocate connection for '" + connKey + "' failed: unexpected exception", e);
        }
        return connRestrictSlot;
    }
}
