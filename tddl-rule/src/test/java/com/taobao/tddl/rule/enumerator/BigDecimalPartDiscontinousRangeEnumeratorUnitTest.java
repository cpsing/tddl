...
public class BigDecimalPartDiscontinousRangeEnumeratorUnitTest {
    ...
    public void test_没有自增的closeinterval() throws Exception {
        btc = gand(gcomp(BigDecimal.valueOf(3), GreaterThan), gcomp(BigDecimal.valueOf(5), LessThanOrEqual));
        try {
    ...
    public void test_一个大于一个null在一个and中() throws Exception {
        btc = gand(gcomp(BigDecimal.valueOf(3), LessThanOrEqual), null);
        try {
            e.getEnumeratedValue(btc, 64, 1, needMergeValueInCloseRange);
        } catch (IllegalArgumentException e) {
            Assert.assertEquals("input value is not a comparative: null", e.getMessage());
        }
    }
    ...
    // btc = gand(gcomp(BigDecimal.valueOf(3),
    // Equivalent),gcomp(BigDecimal.valueOf(5), LessThan));
    // Set<Object> s = e.getEnumeratedValue(btc,
    // 5,1,needMergeValueInCloseRange);
    ...
    // btc = gand(gcomp(BigDecimal.valueOf(3), GreaterThan),gcomp(5l,
    // Equivalent));
    // Set<Object> s = e.getEnumeratedValue(btc,
    // 5,1,needMergeValueInCloseRange);
