...
public class MergeNode extends QueryTreeNode {
    ...
    public IDataNodeExecutor toDataNodeExecutor() {
        ...
        // for (CanOptimizedNode subQuery : this.getChildren()) {
        // merge.addSubNode((subQuery).toDataNodeExecutor());
        // }
        // merge.setOrderBy(this.getOrderBys()).setLimitFrom(this.getLimitFrom()).setLimitTo(this.getLimitTo());
    ...
    public String toString(int inden) {
        ...
        if (this.getAlias() != null) appendln(sb,
            tabTittle + (this.isUnion() ? "Union" : "Merge") + " as " + this.getAlias());
        else appendln(sb, tabTittle + (this.isUnion() ? "Union" : "Merge"));
        ...
        for (Object s : this.getChildren()) {
            ...
            sb.append(node.toString(inden + 1));
        }
