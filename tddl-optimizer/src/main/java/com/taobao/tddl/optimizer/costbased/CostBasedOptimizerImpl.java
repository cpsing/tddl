 ...
 * @since 5.1.0
 */
public class CostBasedOptimizerImpl implements Optimizer {

    private static final Logger                  logger           = LoggerFactory.getLogger(CostBasedOptimizerImpl.class);
    private static Cache<String, OptimizeResult> optimizedResult  = CacheBuilder.newBuilder()
                                                                      .maximumSize(1000)
    ...
    public IDataNodeExecutor optimizeAndAssignment(final ASTNode node,
        ...
        if (cached && sql != null && !sql.isEmpty()) {
            OptimizeResult or;
            try {
                or = optimizedResult.get(sql, new Callable<OptimizeResult>() {
