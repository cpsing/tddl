...
public class MergeNodeCostEstimater implements QueryTreeCostEstimater {
    ...
    public Cost estimate(QueryTreeNode query) throws StatisticsUnavailableException {
        ...
        long networkCost = 0;
        long scanRowCount = 0;
        if (!(merge.getChildren().get(0) instanceof QueryTreeNode)) return cost;
        for (ASTNode sub : merge.getChildren()) {
            Cost subCost = CostEsitimaterFactory.getCostEstimater((QueryTreeNode) sub).estimate((QueryTreeNode) sub);
